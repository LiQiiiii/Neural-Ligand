...
def get_classification_head_dev(args, model, dataset, flag):
    if flag == 'shadow':
        filename = os.path.join(args.save, f'head_{dataset}_shadow.pt')
        if os.path.exists(filename):
            print(f'Classification head for {args.model} on {dataset} exists at {filename}')
            return ClassificationHead.load(filename)
        print(f'Did not find classification head for {args.model} on {dataset} at {filename}, building one from scratch.')
        model_todo = model.model
        template = get_templates(dataset)
        classification_head = build_classification_head(model_todo, dataset, template, args.data_location, args.device)
        os.makedirs(args.save, exist_ok=True)
        classification_head.save(filename)
        return classification_head
    if flag == 'target':
        filename = os.path.join(args.save, f'head_{dataset}_target.pt')
        if os.path.exists(filename):
            print(f'Classification head for {args.model} on {dataset} exists at {filename}')
            return ClassificationHead.load(filename)
        print(f'Did not find classification head for {args.model} on {dataset} at {filename}, building one from scratch.')
