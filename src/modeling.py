...
class ImageEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        ...
        print(f'Creating {args.model} with random initialization.')
        
        self.model, self.train_preprocess, self.val_preprocess = open_clip.create_model_and_transforms(
            args.model, pretrained='openai', cache_dir=args.openclip_cachedir)  # pretrained=None
    ...
    def load_from_state_dict(self, model_name, state_dict):
        print("start loading state dict from {}".format(state_dict))
        self.model, self.train_preprocess, self.val_preprocess = open_clip.create_model_and_transforms(
            model_name, pretrained='openai', device='cpu')
        # model.load_from_state_dict(state_dict, strict=False)
        checkpoint = torch.load(state_dict, map_location=torch.device('cpu'))
        self.model.visual.load_state_dict(checkpoint)
        delattr(self.model, 'transformer')
        print("successfully loading state dict!")
